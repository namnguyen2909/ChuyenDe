...
{
    public partial class GanUser_Quyen : System.Web.UI.Page    
    {
        ...
        {
            bool blQuyen = false;
            int intVaiTroId = CommonFuncs.NUMBER_INVALID_INTEGER;
            if ((!drpDS_Quyen.SelectedValue.Contains(Commons.TitleConst.getTitleConst("BLANK_PARENT_ITEM_VALUE")))
                && (!Commons.TitleConst.getTitleConst("BLANK_ITEM_VALUE").Equals(drpDS_Quyen.SelectedValue)))
            {
                intVaiTroId = Convert.ToInt32(drpDS_Quyen.SelectedValue);
                blQuyen = true;
            }
            ...
            List<int> lstVaiTroUsers = entities.TBL_NGUOI_DUNG_QUYEN.Where(x => (x.ID_QUYEN == intVaiTroId) && (blQuyen)).Select(x => x.ID_NGUOI_DUNG).Distinct().ToList();
        ...
        protected void drpDS_Quyen_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            grvUsers.Selection.UnselectAll();
            ...
            List<int> objs = entities.TBL_NGUOI_DUNG_QUYEN.Where(x => x.ID_QUYEN == intQuyenID).Select(x => x.ID_NGUOI_DUNG).Distinct().ToList();
        ...
        public bool validation()
        {
            ...
            else if (drpDS_Quyen.SelectedValue.Contains(Commons.TitleConst.getTitleConst("BLANK_PARENT_ITEM_VALUE")))
            {
                Commons.ValidationFuncs.errorMessage_TimeDelay(Commons.TitleConst.getTitleConst("MSG_SELECT_CHILD_ITEM"), Page);
                blResult = false;
            }

            btnSave.Enabled = blResult;            
            return blResult;
